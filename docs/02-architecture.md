...
## 2.1 Совместимость PHP

- Runtime-цель MVP: `PHP 7.4+`.
- Код должен быть парсабелен и работоспособен на PHP 7.4.
- На PHP 8.1/8.3 используется тот же кодовый путь без ручного переключения.
- Запрещён синтаксис, который ломает загрузку файла на PHP 7.4.
